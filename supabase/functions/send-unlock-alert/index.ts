...
interface UnlockAlertRequest { email: string; userName?: string; unlockedBy?: string; wasAutoUnlock: boolean; }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { email, userName, unlockedBy, wasAutoUnlock = false }: UnlockAlertRequest = await req.json();
    if (!email) {
    ...
    const unlockMethod = wasAutoUnlock ? "Your account lock has expired and was automatically unlocked." : `Your account has been manually unlocked${unlockedBy ? ' by an administrator' : ''}.`;
      ...
      <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f4f4f5;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f4f4f5; padding: 40px 20px;"><tr><td align="center">
          <table width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            <tr><td style="background: linear-gradient(135deg, #22c55e, #16a34a); padding: 32px; text-align: center;"><div style="font-size: 48px; margin-bottom: 16px;">🔓</div><h1 style="color: #ffffff; margin: 0; font-size: 24px;">Account Unlocked</h1></td></tr>
            <tr><td style="padding: 32px;">
              <p style="color: #18181b; font-size: 16px; margin: 0 0 24px 0;">Hi ${userName || 'there'},</p>
              <div style="background-color: #f0fdf4; border-left: 4px solid #22c55e; padding: 16px; margin-bottom: 24px; border-radius: 0 8px 8px 0;"><p style="color: #166534; margin: 0; font-weight: 600;">✓ ${unlockMethod}</p></div>
              <p style="color: #52525b; font-size: 14px; margin: 0 0 24px 0;">You can now log in to your account again.</p>
              <div style="background-color: #fefce8; border: 1px solid #facc15; border-radius: 8px; padding: 16px; margin-bottom: 24px;"><p style="color: #854d0e; margin: 0 0 12px 0; font-weight: 600;">🛡️ Security Recommendations:</p><ul style="color: #854d0e; margin: 0; padding-left: 20px; font-size: 14px;"><li>Consider changing your password</li><li>Enable two-factor authentication</li><li>Review recent login activity</li></ul></div>
              <div style="background-color: #f4f4f5; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
                <h3 style="color: #18181b; margin: 0 0 16px 0; font-size: 16px;">📊 Account Status</h3>
                <table width="100%">
                  <tr><td style="padding: 8px 0; border-bottom: 1px solid #e4e4e7;"><span style="color: #71717a; font-size: 14px;">Status</span></td><td style="padding: 8px 0; border-bottom: 1px solid #e4e4e7; text-align: right;"><span style="color: #22c55e; font-size: 14px; font-weight: 600;">✓ Active</span></td></tr>
                  <tr><td style="padding: 8px 0; border-bottom: 1px solid #e4e4e7;"><span style="color: #71717a; font-size: 14px;">Unlocked At</span></td><td style="padding: 8px 0; border-bottom: 1px solid #e4e4e7; text-align: right;"><span style="color: #18181b; font-size: 14px; font-weight: 500;">${new Date().toLocaleString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span></td></tr>
                  <tr><td style="padding: 8px 0;"><span style="color: #71717a; font-size: 14px;">Method</span></td><td style="padding: 8px 0; text-align: right;"><span style="color: #18181b; font-size: 14px; font-weight: 500;">${wasAutoUnlock ? 'Automatic' : 'Manual (by admin)'}</span></td></tr>
                </table>
              </div>
            </td></tr>
            <tr><td style="background-color: #f4f4f5; padding: 24px; text-align: center; border-top: 1px solid #e4e4e7;"><p style="color: #71717a; font-size: 12px; margin: 0;">Automated security notice. Do not reply.</p></td></tr>
          </table>
        </td></tr></table>
      </body></html>`;
    ...
    console.error("Error in send-unlock-alert:", error);
