      ...
      try {
        ...
        const methods: EnabledPaymentMethod[] = (data || []).map((config: any) => {
          ...
          return {
            ...
            payment_mode: (() => { try { const c = typeof config.config === 'string' ? JSON.parse(config.config) : config.config; return c?.payment_mode || null; } catch { return null; } })(),
            payable_to: (() => { try { const d = typeof config.account_details === 'string' ? JSON.parse(config.account_details) : config.account_details; return d?.payable_to || null; } catch { return null; } })(),
          };
        });
