...
export function useTabNotifications({ unreadCount, baseTitle = "Messages & Support" }: TabNotificationOptions) {
  ...
  useEffect(() => {
    return () => {
      document.title = originalTitleRef.current || "Messages & Support";
    };
  }, []);
}
