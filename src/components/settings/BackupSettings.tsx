...
export function BackupSettings() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Restore Result Card */}
      {restoreResult && (
        <Card className={restoreResult.missing_tables && restoreResult.missing_tables.length > 0 && restoreResult.total_restored === 0 
          ? "border-destructive/50 bg-destructive/5" 
          : "border-success/50 bg-success/5"}>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2" style={{ color: restoreResult.missing_tables && restoreResult.missing_tables.length > 0 && restoreResult.total_restored === 0 ? 'hsl(var(--destructive))' : 'hsl(var(--success))' }}>
              {restoreResult.total_restored === 0 && restoreResult.missing_tables?.length ? (
                <><AlertTriangle className="h-5 w-5" /> Restore Failed — Tables Missing</>
              ) : (
          ...
          <CardContent>
            {restoreResult.error && restoreResult.total_restored === 0 && (
            ...
            {restoreResult.missing_tables && restoreResult.missing_tables.length > 0 && (
              <Alert variant="destructive" className="mb-4">
                <AlertTriangle className="h-4 w-4" />
                <AlertTitle>{restoreResult.missing_tables.length} Missing Tables</AlertTitle>
                <AlertDescription className="mt-2">
                  <p className="mb-2">These tables don't exist in the database. Run the <strong>Schema SQL</strong> first to create them:</p>
                  <p className="font-mono text-xs bg-destructive/10 p-2 rounded max-h-24 overflow-y-auto">
                    {restoreResult.missing_tables.join(', ')}
                  </p>
                  <p className="mt-2 text-sm">
                    <strong>Steps:</strong> Export Schema (SQL) → Run SQL in new project → Then restore data.
                  </p>
                </AlertDescription>
              </Alert>
            )}
            ...
            {restoreResult.failed_tables && restoreResult.failed_tables.length > 0 && !restoreResult.missing_tables?.length && (
